...
import { Layers, ArrowLeft, CheckCircle2, XCircle, Briefcase, Trophy, Zap, Sliders, CheckSquare, Square } from 'lucide-react';
import Card from '../common/Card';
import Button from '../common/Button';

export const DsComparisonView = ({
  specs = {},
  onBackToSingle
}) => {
  const [selectedKeys, setSelectedKeys] = useState(['array', 'stack', 'singlylist', 'minheap']);
    ...
    <div className="space-y-6 font-body">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-heading font-bold text-textPrimary">
              Data Structure Comparison Studio ({selectedKeys.length} Selected)
            </h3>
            <p className="text-xs text-textSecondary">
              Compare architectural trade-offs, time & space complexity, pros/cons, and industry applications.
            </p>
          </div>
        </div>
      </Card>

      {/* Checkbox Selector for Data Structures */}
      <Card className="p-5 space-y-3">
        <div className="flex items-center justify-between">
          <h4 className="text-xs font-heading font-bold text-textSecondary uppercase tracking-wider flex items-center gap-1.5">
            <Sliders className="w-3.5 h-3.5 text-primary" /> Select 2–6 Data Structures to Compare
          </h4>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between border-b-2 border-borderTheme pb-3">
                  <div>
                    ...
                    <span className="text-[10px] font-mono text-primary font-bold uppercase">{spec.category} Structure</span>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px] font-mono font-bold">
                  <div className="p-2 rounded-xl bg-surface border border-borderTheme">
                    <span className="text-textSecondary text-[9px] block uppercase">Best Operation</span>
                    <span className="text-success">{spec.bestTime}</span>
                  </div>
                  <div className="p-2 rounded-xl bg-surface border border-borderTheme">
                    <span className="text-textSecondary text-[9px] block uppercase">Worst Operation</span>
                    <span className="text-accent">{spec.worstTime}</span>
                  </div>
                </div>
                ...
                <div className="space-y-1.5 pt-2">
                  ...
                  <ul className="space-y-1 text-xs text-textPrimary">
                    {(spec.advantages || ['Fast operations', 'Low overhead']).map((adv, idx) => (
                ...
                <div className="space-y-1.5 pt-2">
                  <span className="text-[11px] font-heading font-bold text-accent uppercase flex items-center gap-1">
                    <XCircle className="w-3.5 h-3.5" /> Disadvantages & Pitfalls
                  </span>
                  <ul className="space-y-1 text-xs text-textSecondary">
                    {(spec.disadvantages || ['Random search penalty', 'Capacity limits']).map((dis, idx) => (
                      <li key={idx} className="flex items-start gap-1.5 text-[11px]">
                ...
                <div className="space-y-1.5 pt-2">
                  <span className="text-[11px] font-heading font-bold text-primary uppercase flex items-center gap-1">
                    <Briefcase className="w-3.5 h-3.5" /> Industry Applications
                  </span>
                  <div className="flex flex-wrap gap-1">
                    {(spec.realWorldUses || ['Call Stacks', 'OS Schedulers']).map((use, idx) => (
      ...
      <Card className="p-5 space-y-4">
        <h4 className="text-xs font-heading font-bold text-textPrimary uppercase tracking-wider flex items-center gap-2">
          <Trophy className="w-4 h-4 text-warning" /> Data Structure Architectural Comparison Matrix
        </h4>
          ...
          <table className="w-full text-left text-xs font-mono border-collapse">
            <thead>
              <tr className="border-b-2 border-borderTheme text-textSecondary uppercase text-[10px]">
                ...
                <th className="py-2.5 px-3">Best Access Time</th>
                <th className="py-2.5 px-3">Worst Search Time</th>
                <th className="py-2.5 px-3">Insertion / Deletion</th>
              ...
              {selectedKeys.map((key) => {
                ...
                return (
                  <tr key={key} className="border-b border-borderTheme hover:bg-surface/50">
                    ...
                    <td className="py-2.5 px-3 font-bold text-success">{spec.bestTime}</td>
                    <td className="py-2.5 px-3 font-bold text-accent">{spec.worstTime}</td>
                    <td className="py-2.5 px-3 font-bold text-info">
                      {key === 'stack' || key === 'queue' || key === 'singlylist' || key === 'deque' ? 'O(1) Ends' : 'O(N) Shift'}
                    </td>
                    <td className="py-2.5 px-3 text-textSecondary">{spec.space}</td>
                    <td className="py-2.5 px-3 text-[11px] text-textPrimary">
                      {(spec.advantages && spec.advantages[0]) || 'Deterministic O(1) operations'}
                    </td>
                  </tr>
                );
              })}
...
export default DsComparisonView;
