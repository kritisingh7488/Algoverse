...
const DATASETS = [
  ...
  { text: 'AAAAAAAAAA', pattern: 'AAA' },
  { text: 'HELLO WORLD', pattern: 'XYZ' } // missing
];
...
export default function StringAutoVerifier({ onClose }) {
    ...
    for (const dataset of DATASETS) {
      for (const algo of ALGORITHMS) {
        try {
          ...
          allResults.push({
            dataset: `T: ${dataset.text.substring(0, 5)}... P: ${dataset.pattern}`,
            algo,
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-bold text-foreground">
                Automated C++ String Algorithms Verification
              </h3>
              <p className="text-xs text-muted">
                Executes automated tests for Naive and KMP string search.
              </p>
            </div>
          </div>
          ...
          {results.map((res, i) => (
            <Card key={i} className={`p-3 border-l-4 rounded-lg flex items-center justify-between ${
              ...
              <div className="flex items-center gap-3 w-1/3">
                ...
                <div>
                  <div className="text-xs font-bold font-mono text-foreground">{res.dataset}</div>
